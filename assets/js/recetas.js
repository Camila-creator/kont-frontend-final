...
// ✅ CAMBIO: Rutas relativas. El apiFetch del main.js ya debe saber hacia dónde apuntar.
const API_PRODUCTS = "/products";
const API_SUPPLIERS = "/supplies";
const API_RECIPES = "/recipes";
...
// 🗑️ BORRADO: La función apiFetch local fue eliminada para usar la global y evitar el error HTML.
...
async function loadSupplies(){
    const res = await apiFetch(API_SUPPLIERS);
    supplies = res.data || res || [];
}
async function loadRecipesForAllProducts(){
    recipesByProductId = new Map();
    for (const p of products) {
        try {
            const res = await apiFetch(`${API_RECIPES}/${p.id}`);
            if (res.data || Array.isArray(res)) {
                recipesByProductId.set(Number(p.id), res.data || res);
            }
        } catch { /* si no existe receta, lo ignoramos silenciosamente */ }
    }
}
...
async function deleteRecipe(productId){
    ...
    try {
        ...
        recipesByProductId.delete(Number(productId)); // Eliminamos del map local también
        renderTable();
    } catch (err) { openAlert({ title:"Error", message: err.message }); }
}
...
function renderTable(){
    ...
    rows.forEach(row=>{
        const statusBadge = row.has 
            ? `<span class="badge badge-ok" style="color: #16a34a; background: #dcfce7; padding: 4px 8px; border-radius: 6px;"><i class="bi bi-check-circle-fill"></i> Lista</span>` 
            : `<span class="badge badge-missing" title="El producto no se puede fabricar sin receta" style="color: #ea580c; background: #ffedd5; padding: 4px 8px; border-radius: 6px;"><i class="bi bi-exclamation-circle-fill"></i> Faltante</span>`;

        const tr = document.createElement("tr");
        tr.style.borderBottom = "1px solid #f1f5f9";
        tr.innerHTML = `
            <td style="padding:15px; font-weight:600; color:#1e293b; vertical-align: middle;">${row.productName}</td>
            <td style="padding:15px; text-align:center; vertical-align: middle;">
                <span class="badge badge-info" style="color: #0284c7; background: #e0f2fe; padding: 4px 8px; border-radius: 6px;">${row.count} insumos</span>
            </td>
            <td style="padding:15px; text-align:center; font-weight:700; color:#64748b; vertical-align: middle;">${row.waste}</td>
            <td style="padding:15px; vertical-align: middle;">${statusBadge}</td>
            <td style="padding:15px; vertical-align: middle; text-align: right;">
                <div class="table-actions">
                    <button class="btn-icon btn-edit" data-action="edit" data-id="${row.productId}" title="Editar Fórmula" style="border: none; background: transparent; cursor: pointer; color: #3b82f6; font-size: 1.2rem; margin-right: 10px;"><i class="bi bi-pencil-square"></i></button>
                    <button class="btn-icon btn-del" data-action="delete" data-id="${row.productId}" title="Eliminar Fórmula" style="border: none; background: transparent; cursor: pointer; color: #ef4444; font-size: 1.2rem;"><i class="bi bi-trash"></i></button>
                </div>
            </td>
        `;
...
// ---------------- INICIO ----------------
async function init(){
    // ✅ Verificación de seguridad: asegurarnos de que el cerebro global está conectado
    if (typeof apiFetch !== "function") {
        console.error("❌ Error Crítico: main.js no detectado. Kont no podrá procesar las fórmulas.");
        return;
    }
